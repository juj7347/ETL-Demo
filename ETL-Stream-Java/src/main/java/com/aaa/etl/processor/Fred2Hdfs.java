...
public class Fred2Hdfs {
	...
	public void writeCsv2Hdfs(String filename, List<EtlColumnPojo> nodeList) throws Exception {
		Path hadooPath = new Path(filename);
		
		FSDataOutputStream hadoopOutputStream = null;
		BufferedWriter bw = null;
		
		if(nodeList.size() != 0) {
			if(hadoopFs.exists(hadooPath)) {
				hadoopOutputStream = hadoopFs.append(hadooPath);
				bw = new BufferedWriter(new OutputStreamWriter(hadoopOutputStream, StandardCharsets.UTF_8));
			} else {
				hadoopOutputStream = hadoopFs.create(hadooPath, true);
				bw = new BufferedWriter(new OutputStreamWriter(hadoopOutputStream,
