...
public class EtlFileUploader2Hdfs {
		...
		private static List<String> titleEarningList = Arrays.asList(
				...
				"Average Hourly Earnings of All Employees: Manufactiruing in ",
		...
		public static void main(String[] args) throws Exception {
			...
			for (US_STATES state: US_STATES.values()) {
				List<EtlColumnPojo> etlUnemployeeDataAnnual = fred.getEtlListData(FREQUENCY.YEAR, state, titleHouseholdIncome);
				fred.writeCsv2Hdfs("unemployee_annual.csv", etlUnemployeeDataAnnual);
				System.out.println("list " + etlUnemployeeDataAnnual);
				if (state.ordinal() == 0) {
			...
			for (US_STATES state: US_STATES.values()) {
				...
				if (state.ordinal() == 0) {
					fred.writeCsv2Local(true, "src/main/outputs/", "household_income.csv", etlhouseholdIncomeData);
				} else {
					fred.writeCsv2Local(false, "src/main/outputs/", "household_income.csv", etlhouseholdIncomeData);
				}
			}
			...
			for (US_STATES state: US_STATES.values()) {
				List<EtlColumnPojo> etlTaxExmpData =
				fred.getEtlListData(FREQUENCY.YEAR, state, titleHouseholdIncome);
				fred.writeCsv2Hdfs("tax_exemption.csv", etlTaxExmpData);
				
				if (state.ordinal() == 0) {
					fred.writeCsv2Local(true, "src/main/outputs/", "tax_exemption.csv", etlTaxExmpData);
				} else {
					fred.writeCsv2Local(false, "src/main/outputs/", "tax_exemption.csv", etlTaxExmpData);
				}
			}
			...
			for (int i = 0; i  < titleEarningList.size(); i ++) {
				fred.clearInputFiles("src/main/outputs/", getFilename(titleEarningList.get(i)));	
			}
			
			for (int i = 0; i < titleEarningList.size(); i++) {
				for (US_STATES state: US_STATES.values()) {
					List<EtlColumnPojo> etlEarningsData =
					fred.getEtlListData(FREQUENCY.YEAR, state, titleEarningList.get(i));
					String filename = getFilename(titleEarningList.get(i));
					
					fred.writeCsv2Hdfs("unemployee_monthly.csv", etlEarningsData);
